...
// First question special behavior: shrinking "No", growing "Yes"
let firstNoClickCount = 0;
...
function handleFirstNo() {
    ...
    // Update "No" button text with different refusal messages
    const msgIndex = Math.min(firstNoClickCount - 1, firstNoMessages.length - 1);
    noBtn.textContent = firstNoMessages[msgIndex];

    // Shrink "No" button until it disappears
    const noScale = Math.max(0.2, 1 - firstNoClickCount * 0.15);
    ...
    const yesScale = 1 + firstNoClickCount * 0.15;
    yesBtn.style.transform = `scale(${yesScale})`;
}

function handleFirstYes() {
    // Proceed to next question when they finally click "Yes"
    showNextQuestion(2);
}
